...
class MySQLBase(ABC):
    ...
    def _build_instance_sock_executor(self, user: str | None = None, passw: str | None = None):
        """Build a socket executor for the instance operations."""
        if not user:
            user = self.root_user
        if not passw:
            passw = self.root_password
    ...
    def check_mysqlsh_connection(self) -> bool:
        ...
        executor = self._build_instance_sock_executor(
            ...
            passw=self.server_config_password,
        )
        try:
            executor.check_connection()
            return True
        except ExecutionError:
            logger.warning(f"Failed to connect to MySQL with user {self.server_config_user}")
        ...
        try:
            ...
            logger.error(f"Failed to connect to MySQL with user {self.root_user}")
