...
class MySQLBase(ABC):
    ...
    def _build_mysql_database_dba_role(self, database: str) -> str:
        ...
        role_name_collisions = self._instance_client_tcp.search_instance_databases(
    ...
    def configure_mysql_router_roles(self) -> None:
        ...
        executor = self._build_instance_sock_executor(self.instance_address)

        for role in (LEGACY_ROLE_ROUTER, MODERN_ROLE_ROUTER):
            if role in self._instance_client_sock.search_instance_roles(role):
                continue
    ...
    def configure_mysql_system_roles(self) -> None:
        ...
        if set(auth_roles) - set(existing_roles) == set():
            return
    ...
    def does_mysql_user_exist(self, username: str, hostname: str) -> bool:
        ...
        try:
            users = self._instance_client_tcp.search_instance_users("%")
        except ExecutionError as e:
    ...
    def create_database(self, database: str) -> None:
        ...
        primary_address = self.get_cluster_primary_address()
        primary_executor = self._build_instance_tcp_executor(primary_address)
        auth_builder = CharmAuthorizationQueryBuilder(
            role_admin=ROLE_DBA,
            role_backup=ROLE_BACKUP,
            role_ddl=ROLE_DDL,
            role_stats=ROLE_STATS,
        ...
        role_query = auth_builder.build_database_admin_role_query(role_name, database)
    ...
    def create_scoped_user(
        ...
        *,
        ...
        try:
            self._instance_client_tcp.create_instance_user(user, password, extra_roles)
            if not extra_roles:
    ...
    def configure_instance(self, create_cluster_admin: bool = True) -> None:
        ...
        try:
            ...
            self._cluster_client_tcp.setup_instance_before_cluster(
    ...
    def cluster_metadata_exists(self, from_instance: str | None = None) -> bool:
        ...
        if from_instance:
            ...
            executor = self._instance_client_sock

        # TODO: simplify when mysql-shell-client 0.4.0 gets released
        query = "SELECT cluster_name FROM mysql_innodb_cluster_metadata.clusters"

        try:
            rows = executor.execute_sql(query)
        except ExecutionError as e:
            logger.warning("Failed to check if cluster metadata exists")
            raise MySQLClusterMetadataExistsError() from e
        else:
            names = [row["cluster_name"] for row in rows]

        return self.cluster_name in names
    ...
    def initialize_juju_units_operations_table(self) -> None:
        ...
        builder = CharmLockingQueryBuilder("mysql", "juju_units_operations")
    ...
    def add_instance_to_cluster(
        ...
        *,
        ...
        primary_address = self.get_cluster_primary_address()
        primary_executor = self._build_cluster_tcp_executor(primary_address)
        primary_client = MySQLClusterClient(primary_executor)
        ...
        try:
            # NOTE: This operation used to be done by the server-config user
            primary_client.attach_instance_into_cluster(
                cluster_name=self.cluster_name,
    ...
    def rejoin_instance_to_cluster(self, *, unit_address: str, unit_label: str) -> None:
        ...
        try:
            # NOTE: This operation used to be done by the server-config user
            primary_client.rejoin_instance_into_cluster(
                cluster_name=self.cluster_name,
    ...
    def is_instance_configured_for_innodb(self, instance_address: str) -> bool:
        ...
        try:
            # NOTE: This operation used to be done by the server-config user
            result = self._cluster_client_tcp.check_instance_before_cluster(
                instance_host=instance_address,
                instance_port=str(3306),
            )
    ...
    def are_locks_acquired(self, task_name: str) -> bool:
        """Report if any topology change is being executed."""
        primary_address = self.get_cluster_primary_address()
        primary_executor = self._build_instance_tcp_executor(primary_address)

        builder = CharmLockingQueryBuilder("mysql", "juju_units_operations")
        query = builder.build_fetch_acquired_query(task_name)

        try:
            logger.debug(f"Fetching acquired lock {task_name}")
            locks = primary_executor.execute_sql(query)
        except ExecutionError:
    ...
    def get_cluster_status(self, extended: bool | None = False) -> dict | None:
        ...
        try:
            # NOTE: This operation used to be done by the server-config user
            status = primary_client.fetch_cluster_status(self.cluster_name, extended)
        except ExecutionError:
            raise
        else:
            return status
    ...
    def get_cluster_set_status(self, extended: int | None = 1) -> dict | None:
        ...
        try:
            # NOTE: This operation used to be done by the server-config user
            status = primary_client.fetch_cluster_status(self.cluster_name, bool(extended))
        except ExecutionError:
            raise
        else:
            return status
    ...
    def get_cluster_node_count(
        ...
        if not node_status:
            node_status = list(InstanceStatus)
        ...
        query = (
            ...
            "WHERE member_state IN ({statuses})"
        )
        query = query.format(statuses=", ".join(node_status))
    ...
    def remove_instance(self, unit_label: str, auto_dissolve: bool | None = True) -> None:
        ...
        try:
            # NOTE: This operation used to be done by the server-config user
            primary_client.detach_instance_from_cluster(
                cluster_name=self.cluster_name,
                instance_host=self.instance_address,
                instance_port=str(3306),
                options=options,
    ...
    @staticmethod
    def _acquire_lock(executor: BaseExecutor, unit_label: str, unit_task: str) -> bool:
        """Attempts to acquire a lock by using the mysql.juju_units_operations table."""
        builder = CharmLockingQueryBuilder("mysql", "juju_units_operations")
        query = builder.build_acquire_query(task=unit_task, instance=unit_label)
    ...
    @staticmethod
    def _release_lock(executor: BaseExecutor, unit_label: str, unit_task: str) -> bool:
        """Releases a lock in the mysql.juju_units_operations table."""
        builder = CharmLockingQueryBuilder("mysql", "juju_units_operations")
        query = builder.build_release_query(task=unit_task, instance=unit_label)
    ...
    def _get_cluster_member_addresses(self, exclude_unit_labels: list) -> list[str]:
        """Get the addresses of the cluster's members."""
        try:
            status = self._cluster_client_tcp.fetch_cluster_status(
                cluster_name=self.cluster_name,
                extended=False,
            )
        except ExecutionError:
        ...
        instances = status["defaultReplicaSet"]["topology"].items()
        instances = [member for label, member in instances if label not in exclude_unit_labels]
        addresses = [member["address"] for member in instances]
    ...
    def is_unit_primary(self, unit_label: str) -> bool:
        ...
        return self.get_primary_label() == unit_label
    ...
    def get_member_state(self) -> tuple[InstanceStatus, InstanceRole]:
    ...
    def tls_setup(
        ...
        tls_var = "ON" if require_tls else "OFF"
        tls_val = "require_secure_transport"
