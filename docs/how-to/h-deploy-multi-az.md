...
Let's destroy GCE node and recreate it using the same AZ (Primary MySQL unit is being removed):
```shell
...
The command `juju status` shows the healthy machines state, but MySQL HA recovery necessary:
```shell
Model    Controller  Cloud/Region     Version    SLA          Timestamp
