...
class MySQLOperatorCharm(MySQLCharmBase, TypedCharmBase[CharmConfig]):
    ...
    def _handle_non_online_instance_status(self, state: str) -> bool:
        ...
        if state == InstanceStatus.OFFLINE:
            ...
            if all_states == {InstanceStatus.OFFLINE} and self.unit.is_leader():
        ...
        if state == InstanceStatus.UNREACHABLE:
            try:
    ...
    def _on_update_status(self, _) -> None:  # noqa: C901
        ...
        if self.unit.is_leader() and state == InstanceStatus.ONLINE:
            try:
    ...
    def join_unit_to_cluster(self) -> None:
        ...
        #  Remove `.lower()` when migrating to MySQL 8.4
        #  (when breaking changes are allowed)
        self.unit_peer_data["member-state"] = InstanceStatus.ONLINE.lower()
        self.unit.status = ActiveStatus(self.active_status_message)
