...
class DBRouterRelation(Object):
    ...
    def _on_db_router_relation_changed(self, event: RelationChangedEvent) -> None:
        ...
        self.charm.unit.status = MaintenanceStatus("Setting up db-router relation")
        ...
        event_relation_databag = event.relation.data[self.charm.unit]
        ...
        for index, application_name in enumerate(application_names):
            ...
            try:
                ...
                if application_name == "mysqlrouter":
                    mysqlrouter_user_exists = self.charm._mysql.does_mysql_user_exist(username)
                    if not mysqlrouter_user_exists:
                        self.charm._mysql.configure_mysqlrouter_user(username, password)
                ...
                if not self.charm._mysql.does_mysql_user_exist(username):
                ...
                self.charm.unit.status = BlockedStatus("Failed to initialize db-router relation")
                return
    ...
    def _on_db_router_relation_broken(self, event: RelationBrokenEvent) -> None:
        ...
        leader_db_router_databag = event.relation.data[self.charm.app]
        ...
        for key in leader_db_router_databag:
            try:
                if key.startswith(app_username_prefix):
                    ...
                    self.charm._mysql.remove_user(app_username)
                ...
                if self.config.get("auto-delete", False) and key.startswith(app_database_prefix):
                    ...
                    self.charm._mysql.remove_database(app_database_name)
            except MySQLRemoveDatabaseError:
                self.charm.unit.status = BlockedStatus("Failed to remove users")
                return
        ...
        try:
            self.charm._mysql.remove_user(mysql_router_username)
        except MySQLRemoveDatabaseError:
            self.charm.unit.status = BlockedStatus("Failed to remove mysqlrouter user")
