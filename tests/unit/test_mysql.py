...
class TestMySQLBase(unittest.TestCase):
    ...
    def test_remove_primary_instance(
        ...
        _get_cluster_primary_address.assert_called_once_with()
        self.mock_executor.execute_py.assert_called_once_with("\n".join(commands))
    ...
    def test_is_instance_in_cluster(self):
    ...
    def test_get_member_state(self):
        ...
        self.assertEqual(state, InstanceStatus.ONLINE)

        self.mock_executor.execute_sql.return_value = [{"member_state": "OFFLINE"}]
        state = self.mysql.get_member_state()
        self.assertEqual(state, InstanceStatus.OFFLINE)
    ...
    @patch("charms.mysql.v0.mysql.MySQLBase.get_cluster_set_global_primary_address")
    def test_update_user_password(self, _get_cluster_set_global_primary_address):
        """Test the successful execution of update_user_password."""
        _get_cluster_set_global_primary_address.return_value = "1.1.1.1"
    ...
    @patch("charms.mysql.v0.mysql.MySQLBase.get_cluster_status")
    def test_get_cluster_node_count(self, _get_cluster_status):
        """Test get_cluster_node_count."""
        _get_cluster_status.return_value = {
            "defaultReplicaSet": {
                "topology": {
                    "mysql-0": {"status": "ONLINE"},
                    "mysql-1": {"status": "ONLINE"},
                    "mysql-2": {"status": "OFFLINE"},
                }
            }
        }

        count = self.mysql.get_cluster_node_count(node_status=InstanceStatus.ONLINE)
        self.assertEqual(count, 2)

        _get_cluster_status.side_effect = ExecutionError
        count = self.mysql.get_cluster_node_count()
        self.assertEqual(count, 0)

    def test_get_cluster_set_global_primary(self):
    ...
    def test_install_plugin(self):
        ...
        self.mock_executor.execute_sql.assert_has_calls([
            call("SET @@GLOBAL.`super_read_only` = 'OFF'"),
            call("SELECT name FROM mysql.plugin WHERE name LIKE '%'"),
            call("SET @@GLOBAL.`super_read_only` = 'ON'"),
        ])
        self.mock_executor.execute_sql.reset_mock()
