...
async def test_kill_db_process(juju: Juju, continuous_writes) -> None:
    ...
    # Ensure continuous writes still incrementing for all healthy units
    healthy_units = set(mysql_units) - {mysql_primary_unit}
    healthy_units = list(healthy_units)
    await check_mysql_units_writes_increment(juju, MYSQL_APP_NAME, healthy_units)
